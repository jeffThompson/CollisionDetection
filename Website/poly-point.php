...
<pre><code>PVector[] vertices = new PVector[4];
</code></pre>
...
<pre><code>vertices[0] = new PVector(200,100);     // set X/Y position
...
<pre><code>boolean collision = false;
</code></pre>
...
<pre><code>PVector vc = vertices[current];    // c for "current"
...
<pre><code>if ( ((vc.y &gt; py) != (vn.y &gt; py)) &amp;&amp; (px &lt; (vn.x-vc.x) * (py-vc.y) / (vn.y-vc.y) + vc.x) ) {
  collision = !collision;
}
</code></pre>
...
<pre><code>px &lt; (vn.x-vc.x) * (py-vc.y) / (vn.y-vc.y) + vc.x
</code></pre>
...
<pre><code>// set collision to the opposite of its current state
collision = !collision;
</code></pre>

<p>Here's a full example with everything together:</p>

<pre><code>float px = 0;    // point position
float py = 0;
